...
const coursesReducer = (state, action) => {
    switch (action.type) {
        case "ADD":
            const id = new Date().getTime().toString() + Math.floor(Math.random() * 10).toString()
        ...
        case "UPDATE":
            const updatedCourseIndex = state.findIndex((course) => course.id === action.payload.id)
...
const CoursesContextProvider = ({ children }) => {
    ...
    return (
        <CoursesContext.Provider value={values}>
            {children}
